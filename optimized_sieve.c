...
//adding 2
void addingtwo(mpz_t z_n[],mpz_t n, int k, int arr[],FILE *fp)
 {
   mpz_t bla;
  mpz_init(bla);
  int cntr=0;
   for(int i=0;i<k;i++)
	...
	cntr++;
     mpz_mod_ui(z_n[i],z_n[i],arr[i]);
         printf("counter=%d\n",cntr);
     fprintf(fp,"%ld",mpz_hamdist (z_n[i], bla));
    }
   mpz_add_ui(n,n,2);
   mpz_clear(bla);
 }
...
void optimized_sieve(mpz_t q, int k, gmp_randstate_t generator, int arr[], int b, int t,FILE *fp)
 {
  ...
  while(test_with_array(z_n,q,k,arr)!=0)
   {
    addingtwo(z_n,q,k,arr,fp);
   }
  while(((mpz_probab_prime_p(q,t)==0))&&(test_with_array(z_n,q,k,arr)!=0))
   {
    addingtwo(z_n,q,k,arr,fp);
   }
